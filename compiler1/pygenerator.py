...
class PyCodeGenerator:
    ...
    def gen_header(self):
        ...
        self.emit("from slangrt import wrap_unsigned, wrap_signed")
        self.emit("")
    ...
    def gen_expression(self, expression: ast.Expression, parens: bool = True) -> str:
        ...
        if isinstance(kind, ast.StringConstant):
            ...
            if expression.ty.is_int():
                if op == "/":
                    res = f"int({res})"
                res = self.wrap_int(res, expression.ty)
            ...
            if kind.ty.is_int():
                return self.wrap_int(f"int({value})", kind.ty)
    ...
    def wrap_int(value, ty):
        signed = ty.is_signed()
        bits = ty.get_bits()
        wrap_func = "wrap_signed" if signed else "wrap_unsigned"
        return f"{wrap_func}({value}, {bits})"
